          ...
          <section class="chat-panel" aria-label="Conversa selecionada">
            @if (selected(); as conversation) {
              ...
              <form class="chat-composer enhanced-chat-composer" (submit)="send($event)">
                <div class="quick-replies" aria-label="Respostas rápidas">@for (reply of quickReplies; track reply) { <button type="button" [disabled]="sending() || !canSendMessage(conversation)" (click)="sendQuickReply(reply)">{{ localization.translate(reply) }}</button> }</div>
...
export class MessagesComponent implements OnInit {
  ...
  connectionStatusLabel(): string { return this.localization.translate(({ connecting: 'Conectando…', live: 'Sincronizado', reconnecting: 'Reconectando…' } as const)[this.connectionState()]); }
  bookingStatusLabel(status: string): string { return this.localization.translate(({ inquiry: 'Contato antes da reserva', confirmed: 'Reserva confirmada', provider_on_the_way: 'A caminho', in_progress: 'Em andamento', completed: 'Serviço concluído', disputed: 'Em análise', cancelled: 'Reserva cancelada', awaiting_confirmation: 'Aguardando confirmação', open: 'Aguardando profissional' } as Record<string, string>)[status] ?? 'Reserva em andamento'); }
