...
export class ProviderProfileComponent implements OnInit {
  ...
  updateService(service: ProviderService, price: string, active: boolean): void { const priceCents = this.toBaseCents(price); if (!Number.isFinite(priceCents) || priceCents < 1000) { this.actionError.set(this.minimumPriceError('Preço mínimo:')); return; } this.perform(this.marketplace.updateProviderService(service.id, { priceCents, active }), (updated) => { this.services.update((items) => items.map((item) => item.id === updated.id ? updated : item)); this.success.set('Serviço atualizado.'); }); }
  addService(id: string, price: string): void { const catalog = this.catalog().find((service) => service.id === id); const priceCents = price ? this.toBaseCents(price) : (catalog?.priceFromCents ?? 0); if (!catalog || !Number.isFinite(priceCents) || priceCents < 1000) { this.actionError.set(this.minimumPriceError('Selecione um serviço. Preço mínimo:')); return; } this.perform(this.marketplace.updateProviderService(id, { priceCents, active: true }), (updated) => { this.services.update((items) => [...items.filter((item) => item.id !== updated.id), updated].sort((a, b) => a.name.localeCompare(b.name))); this.success.set('Serviço adicionado.'); }); }
  ...
  displayPrice(cents: number): number { return Math.round(this.localization.convertAmount(cents / 100, 'BRL') * 100) / 100; }
  minimumDisplayPrice(): number { return Math.ceil(this.localization.convertAmount(10, 'BRL') * 100) / 100; }
  private minimumPriceError(label: string): string { return `${this.localization.translate(label)} ${this.localization.formatMoney(this.minimumDisplayPrice(), this.localization.currency())}`; }
  ...
  private toBaseCents(value: string): number { return Math.round(this.localization.convertAmount(Number(value), this.localization.currency(), 'BRL') * 100); }
}
