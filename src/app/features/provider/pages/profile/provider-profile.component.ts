...
export class ProviderProfileComponent implements OnInit {
  ...
  updateService(service: ProviderService, price: string, active: boolean): void { const priceCents = this.toBaseCents(price); if (!Number.isFinite(priceCents) || priceCents < 1000) { this.actionError.set('O preço mínimo é 10,00 na moeda selecionada.'); return; } this.perform(this.marketplace.updateProviderService(service.id, { priceCents, active }), (updated) => { this.services.update((items) => items.map((item) => item.id === updated.id ? updated : item)); this.success.set('Serviço atualizado.'); }); }
  addService(id: string, price: string): void { const catalog = this.catalog().find((service) => service.id === id); const priceCents = price ? this.toBaseCents(price) : (catalog?.priceFromCents ?? 0); if (!catalog || !Number.isFinite(priceCents) || priceCents < 1000) { this.actionError.set('Selecione um serviço e informe um preço mínimo de 10,00.'); return; } this.perform(this.marketplace.updateProviderService(id, { priceCents, active: true }), (updated) => { this.services.update((items) => [...items.filter((item) => item.id !== updated.id), updated].sort((a, b) => a.name.localeCompare(b.name))); this.success.set('Serviço adicionado.'); }); }
