  ...
  template: `
    ...
    @else if (provider(); as person) {
      <section class="profile-hero profile-showcase-hero"><div class="container profile-hero-inner">
        ...
        <div class="profile-intro"><div class="provider-name-line"><h1 data-cvp-no-localize>{{ person.name }}</h1>@if (person.verified) { <span class="verified" title="Perfil aprovado">✓<span class="sr-only">Perfil aprovado</span></span> }</div><p><span>{{ person.headline }}</span> · <span data-cvp-no-localize>{{ person.city }}</span>@if (person.state) {, <span data-cvp-no-localize>{{ person.state }}</span> }</p><cvp-rating [rating]="person.rating" [count]="person.reviewCount" /></div>
      ...
      <section class="section section-tight"><div class="container detail-layout">
        <div class="detail-main">
          ...
          <section class="content-card feedback-section"><div class="section-heading"><div><span class="eyebrow">Reservas concluídas</span><h2>Avaliações verificadas</h2><p>Somente clientes que fecharam negócio pela plataforma podem avaliar com estrelas.</p></div><cvp-rating [rating]="person.rating" [count]="person.reviewCount" /></div>
            @if (reviews().length) { <div class="review-list">@for (review of reviews(); track review.id) { <article><div class="review-head"><span class="avatar avatar-sm">{{ review.initials }}</span><div><strong data-cvp-no-localize>{{ review.author }}</strong><span><span class="rating"><span aria-hidden="true">★</span> {{ review.rating }}</span><em class="verified-transaction-badge">Reserva verificada</em></span></div><time [attr.datetime]="review.createdAt">{{ review.createdAt | appDate:'d MMM yyyy' }}</time></div>@if (review.serviceName) { <small class="review-service">{{ review.serviceName }}</small> }<p>{{ review.comment }}</p>@if (review.providerReply) { <div class="provider-reply"><strong>Resposta do profissional</strong><p>{{ review.providerReply }}</p></div> }@if (auth.user()) { <button class="text-button" type="button" (click)="startReport('review', review.id)">Denunciar avaliação</button> }</article> }</div>@if (reviewPage() < reviewLastPage()) { <div class="feedback-load-more"><button class="btn btn-secondary btn-small" type="button" [disabled]="loadingMoreReviews()" (click)="loadMoreReviews()">{{ loadingMoreReviews() ? 'Carregando…' : 'Ver mais avaliações' }}</button></div> } } @else { <p class="muted">As avaliações de reservas concluídas aparecerão aqui.</p> }
          </section>
          ...
          <section class="content-card feedback-section community-comments"><div class="section-heading"><div><span class="eyebrow">Comunidade</span><h2>Comentários públicos</h2><p>Espaço para comentários de membros da plataforma. Não altera a nota profissional.</p></div><span class="comment-count">{{ commentTotal() }} comentário{{ commentTotal() === 1 ? '' : 's' }}</span></div>
            ...
            @if (comments().length) { <div class="review-list comment-list">@for (comment of comments(); track comment.id) { <article><div class="review-head"><span class="avatar avatar-sm">{{ comment.initials }}</span><div><strong data-cvp-no-localize>{{ comment.author }}</strong><span>Comentário da comunidade</span></div><time [attr.datetime]="comment.createdAt">{{ comment.createdAt | appDate:'d MMM yyyy' }}</time></div><p>{{ comment.comment }}</p>@if (auth.user()) { <button class="text-button" type="button" (click)="startReport('professional_comment', comment.id)">Denunciar comentário</button> }</article> }</div>@if (commentPage() < commentLastPage()) { <div class="feedback-load-more"><button class="btn btn-secondary btn-small" type="button" [disabled]="loadingMoreComments()" (click)="loadMoreComments()">{{ loadingMoreComments() ? 'Carregando…' : 'Ver mais comentários' }}</button></div> } } @else { <p class="muted">Ainda não há comentários públicos.</p> }
          </section>
        </div>
