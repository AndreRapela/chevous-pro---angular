...
@Component({
  ...
  standalone: true,
  imports: [AvatarComponent, LocalizedDatePipe, LocalizedMoneyPipe, LocalizedNumberPipe, RatingComponent, RouterLink, StatePanelComponent],
  template: `
    ...
    @else if (provider(); as person) {
      <section class="profile-hero profile-showcase-hero"><div class="container profile-hero-inner">
        ...
        <div class="profile-intro"><div class="provider-name-line"><h1 data-cvp-no-localize>{{ person.name }}</h1>@if (person.verified) { <span class="verified" title="Perfil aprovado">✓<span class="sr-only">Perfil aprovado</span></span> }</div><p><span data-cvp-no-localize>{{ person.headline }}</span> · <span data-cvp-no-localize>{{ person.city }}</span>@if (person.state) {, <span data-cvp-no-localize>{{ person.state }}</span> }</p><cvp-rating [rating]="person.rating" [count]="person.reviewCount" /><div class="chip-row"><span class="chip chip-soft"><span>{{ person.completedJobs }}</span> <span>serviços concluídos</span></span>@if (person.yearsExperience) { <span class="chip chip-soft"><span>{{ person.yearsExperience }}</span> <span>anos de experiência</span></span> }@for (quality of person.qualities.slice(0, 2); track quality) { <span class="chip chip-soft" data-cvp-no-localize>{{ quality }}</span> }</div></div>
      ...
      <section class="section section-tight"><div class="container detail-layout">
        <div class="detail-main">
          ...
          <section class="content-card"><h2>Serviços oferecidos</h2><div class="compact-service-list">@for (service of offeredServices(); track service.id) { <a [routerLink]="['/servicos', service.slug]"><span class="service-symbol">{{ service.symbol }}</span><span><strong>{{ service.name }}</strong><small>{{ service.description }}</small></span><span>→</span></a> }</div></section>
        ...
        <aside class="booking-aside"><span class="eyebrow">Próximo horário</span><strong data-cvp-no-localize>{{ person.nextAvailability }}</strong><p>@if (person.priceFromCents > 0) { A partir de <b>{{ person.priceFromCents / 100 | appMoney:'BRL':0 }}</b> } @else { <b>Preço calculado na reserva</b> }</p><a class="btn btn-primary btn-block" [routerLink]="['/agendar', defaultServiceId()]" [queryParams]="{ profissional: person.id }">Escolher este profissional</a><small>O valor de referência e os detalhes ficam disponíveis antes da confirmação.</small></aside>
      </div></section>
      <div class="mobile-sticky-action"><div><small>{{ person.priceFromCents > 0 ? 'A partir de' : 'Valor' }}</small><strong>{{ person.priceFromCents > 0 ? (person.priceFromCents / 100 | appMoney:'BRL':0) : 'na reserva' }}</strong></div><a class="btn btn-primary" [routerLink]="['/agendar', defaultServiceId()]" [queryParams]="{ profissional: person.id }">Agendar</a></div>
    }
  `,
