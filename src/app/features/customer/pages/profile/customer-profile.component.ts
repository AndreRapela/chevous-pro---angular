...
export class CustomerProfileComponent implements OnInit {
  ...
  editAddress(address: Address): void {
    ...
    this.addressForm.reset({ label: address.label || 'Casa', postalCode: address.postalCode, street: address.street, number: address.number, complement: address.complement || '', neighborhood: address.neighborhood, city: address.city, state: address.state, isDefault: !!address.isDefault });
  }

  resetAddressForm(): void {
    this.editingAddressId.set(''); this.removeTarget.set('');
    this.addressForm.reset({ label: 'Casa', postalCode: '', street: '', number: '', complement: '', neighborhood: '', city: this.auth.user()?.city || 'São Paulo', state: 'SP', isDefault: false });
  }
