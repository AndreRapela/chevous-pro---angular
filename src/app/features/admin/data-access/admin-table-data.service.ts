...
export class AdminTableDataService {
  ...
  load(key: AdminPageKey, page = 1): Observable<AdminTableResult> {
    ...
    if (key === 'finance') {
      return this.marketplace.adminPayments({ page }).pipe(map((response) => this.result(response.data.map((item) => this.paymentRow(item)), response.meta)));
    }
    if (key === 'coupons') {
      return this.marketplace.adminCoupons({ page }).pipe(map((response) => this.result(response.data.map((item) => this.couponRow(item)), response.meta)));
    }
  ...
  private bookingRow(value: unknown): AdminRow { const item = this.record(value); const status = this.text(item, 'status'); return { id: this.text(item, 'id'), primary: this.text(item, 'serviceName', 'Serviço'), secondary: this.shortId(this.text(item, 'id')), cells: [`${this.text(item, 'customerName')} / ${this.text(item, 'professionalName', 'A definir')}`, this.date(this.text(item, 'scheduledStart')), this.money(this.amount(item, 'totalCents'))], status: this.label(status), tone: this.statusTone(status), rawStatus: status }; }
  ...
  private paymentRow(value: unknown): AdminRow { const item = this.record(value); const status = this.text(item, 'status'); return { id: this.text(item, 'id'), primary: this.shortId(this.text(item, 'id')), secondary: status, cells: [this.shortId(this.text(item, 'bookingId')), this.date(this.text(item, 'createdAt')), this.money(this.amount(item, 'amountCents'))], status: this.label(status), tone: this.statusTone(status), rawStatus: status }; }
  private couponRow(value: unknown): AdminRow { const item = this.record(value); const active = Boolean(item['active']); const type = this.text(item, 'discountType'); const discount = type === 'percent' ? `${this.amount(item, 'discountValue')}%` : this.money(this.amount(item, 'discountValue')); return { id: this.text(item, 'id'), primary: this.text(item, 'code'), secondary: this.text(item, 'name'), cells: [discount, `${this.amount(item, 'usedCount')} / ${this.amount(item, 'usageLimit') || '∞'}`, `${this.date(this.text(item, 'startsAt'))} — ${this.date(this.text(item, 'endsAt'))}`], status: active ? 'Ativo' : 'Inativo', tone: active ? 'success' : 'neutral', rawStatus: active ? 'active' : 'inactive' }; }
  ...
  private money(cents: number): string { return new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(cents / 100); }
  private date(value: string): string { const normalized = /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(value) ? `${value.replace(' ', 'T')}Z` : value; const date = new Date(normalized); return Number.isNaN(date.getTime()) ? '—' : new Intl.DateTimeFormat('pt-BR', { dateStyle: 'short', timeStyle: 'short' }).format(date); }
  ...
  private statusTone(status: string): AdminTone { return ['paid', 'completed', 'confirmed', 'active', 'approved'].includes(status) ? 'success' : ['failed', 'cancelled', 'suspended', 'rejected', 'disputed'].includes(status) ? 'danger' : ['pending', 'awaiting_payment', 'awaiting_confirmation', 'open'].includes(status) ? 'warning' : 'neutral'; }
