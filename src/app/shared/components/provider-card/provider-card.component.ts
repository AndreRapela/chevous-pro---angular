...
@Component({
  ...
  standalone: true,
  imports: [AvatarComponent, CurrencyPipe, RatingComponent, RouterLink],
  template: `
    <article class="provider-card">
      <div class="provider-card-head">
        <cvp-avatar [initials]="provider.initials" size="lg" />
        <div>
      ...
      <div class="card-footer">
        @if (provider.priceFromCents > 0) { <span>A partir de <strong>{{ provider.priceFromCents / 100 | currency:'BRL':'symbol':'1.0-0':'pt-BR' }}</strong></span> } @else { <span><strong>Preço na proposta</strong></span> }
        <a class="btn btn-small btn-secondary" [routerLink]="['/profissionais', provider.id]">Ver perfil</a>
      </div>
    </article>
  `,
