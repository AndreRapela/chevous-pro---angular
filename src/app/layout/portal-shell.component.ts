...
const NAVIGATION: Record<string, NavItem[]> = {
  ...
  provider: [
    ...
    { label: 'Ganhos e repasses', short: 'Ganhos', path: '/prestador/ganhos', symbol: '$' },
  ...
  admin: [
    ...
    { label: 'Financeiro', short: 'Financeiro', path: '/admin/financeiro', symbol: '$' },
    { label: 'Cupons', short: 'Cupons', path: '/admin/cupons', symbol: '%' },
    { label: 'Suporte', short: 'Suporte', path: '/admin/suporte', symbol: '?' },
...
@Component({
  ...
  standalone: true,
  imports: [BrandComponent, DatePipe, RouterLink, RouterLinkActive, RouterOutlet],
  template: `
    ...
    <div class="portal-layout" [class.admin-layout]="portal === 'admin'">
      ...
      <nav class="bottom-nav portal-bottom-nav" aria-label="Atalhos do painel">
        ...
        <details class="bottom-more"><summary><span class="portal-nav-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><circle cx="5" cy="12" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="19" cy="12" r="1.5"/></svg></span><span class="bottom-label">Mais</span></summary><div>@for (item of navigation.slice(4); track item.path) { <a [routerLink]="item.path" routerLinkActive="active"><span aria-hidden="true">{{ item.symbol }}</span>{{ item.label }}</a> }</div></details>
      </nav>
    </div>
  `,
...
export class PortalShellComponent implements OnInit {
  ...
  loadNotifications(): void {
    ...
    this.marketplace.notifications({ perPage: 30 }).subscribe({
  ...
  read(item: AppNotification): void {
    if (item.readAt) return;
    this.marketplace.readNotification(item.id).subscribe({ next: () => { this.notifications.update((items) => items.map((current) => current.id === item.id ? { ...current, readAt: new Date().toISOString() } : current)); this.unreadCount.update((value) => Math.max(0, value - 1)); } });
  }
